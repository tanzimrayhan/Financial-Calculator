...
namespace FinancialCalculations
{
    public class Calculations
	{
        ...
        public static decimal CalculateNumberOfPayments(decimal monthlyInterestRate,
            decimal monthlyPay, decimal loanAmount)
        {
            double dMonthlyInterestRate = Convert.ToDouble(monthlyInterestRate);
            double dMonthlyPay = Convert.ToDouble(monthlyPay);
            double dLoanAmount = Convert.ToDouble(loanAmount);
            double dNumberOfPayments = ((-1) * Math.Log(1 - ((dMonthlyInterestRate * dLoanAmount) / dMonthlyPay))) / Math.Log(1 + dMonthlyInterestRate);
           
            decimal numberOfPayments = Convert.ToDecimal(dNumberOfPayments);
